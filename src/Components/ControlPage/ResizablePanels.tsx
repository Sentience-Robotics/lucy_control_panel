...
import { UI_ACCENT_GREEN, UI_BORDER_MUTED, UI_BORDER_STRONG, UI_NAV_BAR_BG } from '../../Constants/uiTheme';
...
const ResizablePanels = ({
  ...
  const [sizes, setSizes] = useState<number[]>(() =>
    proportions.length === count ? normalize(proportions) : normalize(Array(count).fill(1))
  );
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      const deltaPct = ((currentPos - startPos.current) / totalSize) * 100;
    ...
    >
      {panels.map((panel, i) => (
        <Fragment key={i}>
          ...
          {i < panels.length - 1 && (() => {
            const isEngaged = hoverIndex === i || activeIndex === i;
            return (
              <div
              ...
              >
                <div
                  style={{
                    ...
                    width: isHorizontal ? `${isEngaged ? lineThickness + 2 : lineThickness}px` : '100%',
                    height: isHorizontal ? '100%' : `${isEngaged ? lineThickness + 2 : lineThickness}px`,
                    transform: isHorizontal ? 'translateX(-50%)' : 'translateY(-50%)',
                    background: isEngaged ? UI_ACCENT_GREEN : UI_BORDER_STRONG,
